...
class GB_GA_Generator(GoalDirectedGenerator):
    ...
    def generate_optimized_molecules(self, scoring_function: ScoringFunction, number_molecules: int,
        ...
        population_smiles = [x for _, x in sorted(zip(starting_scores,starting_population), key=lambda pair: pair[0], reverse=True)]
        ...
        population_history = []
        population_history.append([Chem.MolToSmiles(m) for m in population_mol])
        ...
        for generation in range(self.generations):
            ...
            population_scores = scoring_function.score_list([Chem.MolToSmiles(m) for m in population_mol])
