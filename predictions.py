...
from utils import ecfp, timestamp

chids = os.listdir('./results/goal_directed_v2/graph_ga')
...
for chid in chids:
    ...
    for i in range(10):
        ...
        y2s.append(y2)
        clfs = {}
        clfs['Split1'] = RandomForestClassifier(n_estimators=n_estimators, n_jobs=n_jobs)
        clfs['Split1'].fit(X1, y1)

        clfs['Split2'] = RandomForestClassifier(n_estimators=n_estimators, n_jobs=n_jobs)
    ...
    trainset_predictions[chid] = predictions

with open(f'results/predictions/{timestamp()}_predictions.p', 'wb') as f:
